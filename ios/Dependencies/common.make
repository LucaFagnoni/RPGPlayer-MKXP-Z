...
# Explicitly including freetype2 dir for now
CFLAGS := -I$(INCLUDEDIR) -I$(INCLUDEDIR)/freetype2 $(TARGETFLAGS) $(DEFINES) -O3
LDFLAGS := -L$(LIBDIR)

# Use Xcode's iOS toolchain
CC      := $(shell xcrun -sdk iphoneos -find clang) -arch $(ARCH)
CXX     := $(shell xcrun -sdk iphoneos -find clang++) -arch $(ARCH)
...
# Ruby configuration for iOS cross-compilation
# Disabling things that usually fail on iOS
RUBY_CONFIGURE_ARGS := \
